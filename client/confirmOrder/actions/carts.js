...
export const settleCart = (cartIds) => dispatch =>{
	...
	shop.settleCart(cartIds,function(){
		...
		browserHistory.push({pathname:"/confirmOrderWap"});
	});
}
//提交订单
export const handleSubmitOrder = (remark) => dispatch =>{
    console.log('ss')
	shop.handleSubmitOrder(remark,(order_data) => {
        if(order_data){
            browserHistory.push({ pathname: 'wxpay',query: {order_sn:order_data.order_sn,user_name:order_data.user_name,total_fee:order_data.total_fee} });
        }else{
            alert('下单失败');
        }
    }
     //    function(order_sn,user_name,total_fee){
